...
function analyze_mesh(distances::Matrix{Float64})
    ...
    println(
        "Shortest edge in the mesh: $(RED)$(BOLD)$(round(shortest_edge, digits=4))$(RESET)",
    )
    ...
    println(
        "Average edge length of the smallest element: $(RED)$(BOLD)$(round(avg_edge_smallest, digits=4))$(RESET)",
    )
    println(
        "Average edge length of the largest element: ",
        round(avg_edge_largest, digits = 4),
    )
end
