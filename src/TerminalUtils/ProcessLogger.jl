...
function initialize_logger(
    filename::String = "computation_log.txt",
...
function log_message(config::LoggerConfig, level::Symbol, message::String)
    ...
    level_hierarchy = Dict(:error => 1, :warning => 2, :info => 3, :success => 4)
    ...
    timestamp =
        config.include_timestamp ? "[$(Dates.format(now(), "yyyy-mm-dd HH:MM:SS"))] " : ""
...
function log_computation_progress(
    config::LoggerConfig,
    element_number::Int,
    total_elements::Int,
    ...
    log_info(
        config,
        "Progress: $(progress)% - Element: $(element_number)/$(total_elements) - Current Volume: $(current_volume)",
    )
end
