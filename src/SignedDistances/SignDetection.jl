...
function SignDetection(mesh::Mesh, grid::Grid, points::Matrix)
    ...
    @threads for batch = 1:num_batches
...
function create_grid_tetrahedra_mapping(mesh::Mesh, grid::Grid, grid_dims)
    ...
    local_grids = [
        [Vector{Int}() for _ = 1:grid_dims[1], _ = 1:grid_dims[2], _ = 1:grid_dims[3]] for _ = 1:num_threads
    ]
    ...
    @threads for el = 1:nel
        ...
        for i = min_idx[1]:max_idx[1]
            for j = min_idx[2]:max_idx[2]
                for k = min_idx[3]:max_idx[3]
    ...
    grid_tetrahedra =
        [Vector{Int}() for _ = 1:grid_dims[1], _ = 1:grid_dims[2], _ = 1:grid_dims[3]]
...
"""
function is_point_in_tetrahedron_fastest(
    tetrahedron::AbstractMatrix{Float64},
